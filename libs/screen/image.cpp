...
ImageG_ mkImage(int width, int height, int bpp) {
    if (width < 0 || height < 0 || width > 2000 || height > 2000)
        return NULL;
    if (bpp != 1 && bpp != 4)
        return NULL;
    uint32_t sz = byteSize(width, height, bpp);
    ImageG_ r = allocImage(NULL, sz);
    auto hd = r->header();
...
namespace ImageGMethods {

/**
 * Get the width of the image
 */
//% property
int width(ImageG_ img) {
    return img->width();
}
 ...
 */
//% property
bool isMono(ImageG_ img) {
    return img->bpp() == 1;
}

//% property
bool isStatic(ImageG_ img) {
    return img->buffer->isReadOnly();
}

//% property
bool revision(ImageG_ img) {
    return img->revision;
}
 ...
 */
//%
void copyFrom(ImageG_ img, ImageG_ from) {
...
static void setCore(ImageG_ img, int x, int y, int c) {
...
static int getCore(ImageG_ img, int x, int y) {
    auto ptr = img->pix(x, y);
    if (img->bpp() == 4) {
        if (y & 1)
            return *ptr >> 4;
 ...
 */
//%
void setPixel(ImageG_ img, int x, int y, int c) {
    if (!img->inRange(x, y))
        return;
    img->makeWritable();
    setCore(img, x, y, c);
 ...
 */
//%
int getPixel(ImageG_ img, int x, int y) {
    if (!img->inRange(x, y))
        return 0;
    return getCore(img, x, y);
}

void fillRect(ImageG_ img, int x, int y, int w, int h, int c);

/**
 * Fill entire image with a given color
 */
//%
void fill(ImageG_ img, int c) {
    if (c && img->hasPadding()) {
 ...
 */
//%
void getRows(ImageG_ img, int x, Buffer dst) {
    if (img->bpp() != 4)
        return;
 ...
 */
//%
void setRows(ImageG_ img, int x, Buffer src) {
    if (img->bpp() != 4)
        return;
...
//%
void _fillRect(ImageG_ img, int xy, int wh, int c) {
    fillRect(img, XX(xy), YY(xy), XX(wh), YY(wh), c);
}

void mapRect(ImageG_ img, int x, int y, int w, int h, Buffer map) {
    if (w == 0 || h == 0 || x >= img->width() || y >= img->height())
        return;
...
//%
void _mapRect(ImageG_ img, int xy, int wh, Buffer c) {
    mapRect(img, XX(xy), YY(xy), XX(wh), YY(wh), c);
}

//% argsNullable
bool equals(ImageG_ img, ImageG_ other) {
    if (!other) {
        return false;
    }
    auto len = img->length();
 ...
 */
//%
ImageG_ transposed(ImageG_ img) {
    ImageG_ r = mkImage(img->height(), img->width(), img->bpp());
...
void drawImage(ImageG_ img, ImageG_ from, int x, int y);

/**
 * Every pixel in image is moved by (dx,dy)
 */
//%
void scroll(ImageG_ img, int dx, int dy) {
    img->makeWritable();
    auto bh = img->byteHeight();
    auto w = img->width();
    if (dy != 0) {
 ...
 */
//%
ImageG_ doubledX(ImageG_ img) {
    if (img->width() > 126)
        return NULL;

    ImageG_ r = mkImage(img->width() * 2, img->height(), img->bpp());
    auto src = img->pix();
    auto dst = r->pix();
    auto w = img->width();
    auto bh = img->byteHeight();
 ...
 */
//%
ImageG_ doubledY(ImageG_ img) {
    if (img->height() > 126)
        return NULL;

    ImageG_ r = mkImage(img->width(), img->height() * 2, img->bpp());
    auto src0 = img->pix();
    auto dst = r->pix();
 ...
 */
//%
void replace(ImageG_ img, int from, int to) {
    if (img->bpp() != 4)
        return;
    to &= 0xf;
    if (from == to)
 ...
 */
//%
ImageG_ doubled(ImageG_ img) {
    ImageG_ tmp = doubledX(img);
    registerGCObj(tmp);
    ImageG_ r = doubledY(tmp);
    unregisterGCObj(tmp);
    return r;
}

bool drawImageCore(ImageG_ img, ImageG_ from, int x, int y, int color) {
    auto w = from->width();
    auto h = from->height();
    auto sh = img->height();
    auto sw = img->width();
 ...
 */
//%
void drawTransparentImage(ImageG_ img, ImageG_ from, int x, int y) {
    img->makeWritable();
    drawImageCore(img, from, x, y, 0);
}
 ...
 */
//%
bool overlapsWith(ImageG_ img, ImageG_ other, int x, int y) {
    return drawImageCore(img, other, x, y, -1);
}

// ImageG_ format (legacy)
...
//%
void _drawIcon(ImageG_ img, Buffer icon, int xy, int c) {
    img->makeWritable();
...
static void drawLineLow(ImageG_ img, int x0, int y0, int x1, int y1, int c) {
    int dx = x1 - x0;
    int dy = y1 - y0;
    int yi = 1;
    if (dy < 0) {
...
static void drawLineHigh(ImageG_ img, int x0, int y0, int x1, int y1, int c) {
    int dx = x1 - x0;
    int dy = y1 - y0;
    int xi = 1;
    if (dx < 0) {
...
void drawLine(ImageG_ img, int x0, int y0, int x1, int y1, int c) {
    if (x1 < x0) {
...
//%
void _drawLine(ImageG_ img, int xy, int wh, int c) {
    drawLine(img, XX(xy), YY(xy), XX(wh), YY(wh), c);
}

void blitRow(ImageG_ img, int x, int y, ImageG_ from, int fromX, int fromH) {
...
//%
void _blitRow(ImageG_ img, int xy, ImageG_ from, int xh) {
    blitRow(img, XX(xy), YY(xy), from, XX(xh), YY(xh));
}

bool blit(ImageG_ dst, ImageG_ src, pxt::RefCollection *args) {
...
//%
bool _blit(ImageG_ img, ImageG_ src, pxt::RefCollection *args) {
    return blit(img, src, args);
}

void fillCircle(ImageG_ img, int cx, int cy, int r, int c) {
    int x = r - 1;
    int y = 0;
    int dx = 1;
    int dy = 1;
...
//%
void _fillCircle(ImageG_ img, int cxy, int r, int c) {
 ...
 */
//%
ImageG_ create(int width, int height) {
    ImageG_ r = mkImage(width, height, IMAGE_BITS);
    if (r)
 ...
 */
//%
ImageG_ ofBuffer(Buffer buf) {
    return ImageGMethods::convertAndWrap(buf);
}
